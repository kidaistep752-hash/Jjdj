-- ====================================================================
--  PART 1: SERVICES & MASTER INITIALIZATION
-- ====================================================================
...
-- ====================================================================
--  PART 2: AUTO-SAVE ENGINE & MOBILE DRAGGING
-- ====================================================================
...
-- ====================================================================
--  PART 3: ROUNDED UI WINDOW & OPENING TOGGLE
-- ====================================================================
...
-- ====================================================================
--  PART 5: PLAYER TAB ELEMENTS (SPEED, SPIN, GODMODES)
-- ====================================================================
...
-- ====================================================================
--  PART 6: GAME AND OTHER TABS FEATURES (TARGET & MATCHMAKING)
-- ====================================================================
...
PrivateBtn.MouseButton1Click:Connect(function()
    PrivateBtn.Text = "Searching for empty server..."
    PrivateBtn.TextColor3 = Color3.fromRGB(186, 85, 211)
    saveSettings()
    local HttpApi = "https://roblox.com" .. game.PlaceId .. "/servers/Public?sortOrder=Asc&limit=100"
    local success, result = pcall(function() return HttpService:JSONDecode(game:HttpGet(HttpApi)) end)
    if success and result and result.data then
        local targetServerId = nil
        for _, server in pairs(result.data) do
            if server.playing and server.playing == 0 and server.id ~= game.JobId then targetServerId = server.id break end
        end
        if not targetServerId then
            for _, server in pairs(result.data) do
                if server.playing and server.playing <= 1 and server.id ~= game.JobId then targetServerId = server.id break end
            end
        end
        if targetServerId then
            TeleportService:TeleportToPlaceInstance(game.PlaceId, targetServerId, LocalPlayer)
        else
            PrivateBtn.Text = "No empty servers found! Retrying..."
            task.wait(1.5)
            TeleportService:Teleport(game.PlaceId, LocalPlayer)
        end
    else
        PrivateBtn.Text = "API Error! Standard Teleporting..."
        task.wait(1)
        TeleportService:Teleport(game.PlaceId, LocalPlayer)
    end
end)
...
-- ====================================================================
--  PART 7: BACKGROUND RUNTIME TICK LOOPS
-- ====================================================================
